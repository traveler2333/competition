...
def calculate_metrics_complete(df):
    ...
    for _, match_group in df.groupby(['match_id']):
        ...
        for _, set_group in match_group.groupby(['set_no']):
            ...
            M_previous = 0 if first_set_in_match else df.at[set_group.index[0] - 1, 'p2_Momentum'] 
            ...
            for i in set_group.index:
                ...
                # 计算S_i
                S_i = 1.17 if row['server'] == 2 else 1
                ...
                # 更新得分情况
                P_i = 1 if row['point_victor'] == 2 else -1
                ...
                # 计算当前点的势头值
                M_i = 0.9*M_previous + S_i * C_t * P_i-0.2*df.at[i, 'p1_double_fault']-0.1*df.at[i, 'p1_break_pt_won']+0.1*df.at[i, 'p2_break_pt_won']
                df.at[i, 'p2_Momentum'] = M_i
